...
图片配准控件，支持上传图片并通过拖拽 4 个角点进行地理配准，输出配准坐标和变换后的图片。支持单图导出、网格切片导出和 ZIP 打包下载。

> 适用于将航拍照片、扫描地图、历史影像等非地理参考图片叠加到地图上，通过手动对齐角点完成地理配准。
...
| `corners` | `GeoCorners` | - | 受控模式：外部传入四角地理坐标 |
| `defaultCorners` | `GeoCorners` | - | 非受控模式：初始四角地理坐标 |
...
```tsx
// 四角地理坐标 [topLeft, topRight, bottomRight, bottomLeft]
type GeoCorners = [
  [number, number],  // TL [lng, lat]
  [number, number],  // TR [lng, lat]
  [number, number],  // BR [lng, lat]
  [number, number],  // BL [lng, lat]
]
```
...
interface ExportConfig {
  ...
  cols?: number          // 切片列数，默认 1
  rows?: number          // 切片行数，默认 1
...
通过 `ref` 获取 `ImageCalibrationHandle`：
...
calibRef.current?.setCorners([
  [116.3, 40.0], [116.5, 40.0],
  [116.5, 39.8], [116.3, 39.8],
])
